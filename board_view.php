            ...
            <div class="main_content">
                ...
                <div class="guide_content">
                    ...
                    <div class="board_btn">
                        ...
                        <a href="./guide3.php?page=1"><button>목록</button></a>
                    </div>
                    ...
                    <?php
                        ...
                        if(isset($_POST['modifyPw'])){ // 만약 modifyPw 값이 있다면
                            ...
                            if(password_verify($modiPw, $encrypted_pw)){ // modiPw와 DB에 저장된 비밀번호가 같다면 
                                echo "<script>
                                location.replace('./board_modify.php?num=".$num."');
                                </script>";
                            } else {
                                echo "<script>
                        ...
                        if(isset($_POST['deletePw'])){ // 만약 deletePw 값이 있다면 
                            ...
                            if(password_verify($deletePw, $encrypted_pw)){ // deletePw와 DB에 저장된 비밀번호가 같다면 
                                echo "<script>
                                location.replace('./board_delete_ok.php?num=".$num."');
                                </script>";
                            } else {
                                echo "<script>
                                alert('비밀번호가 일치하지 않습니다.');
                    ...
                    <div class="reply">
                        ...
                        <div class="reply_form">
                            <form action="./board_reply.php?num=<?php $num = $_GET['num']; echo $num; ?>" method="post">
                                <div class="reply_form_user">
                                    <input type="text" name="board_reply_name" placeholder="작성자" required>
                                    <input type="password" name="board_reply_pw" placeholder="비밀번호" required> 
                                </div>
                                <textarea name="board_reply_content"cols="30" rows="10" placeholder="댓글을 작성해주세요" spellcheck="false"></textarea> <br>
                        ...
                        <div class="reply_read">
                            <ul>
                                ...
                                if(!$conn) {
                                    ...
                                    $num = $_GET['num'];
                                $replySql = "SELECT * FROM boardReply WHERE num=$num ORDER BY replyNum DESC";
                                ...
                                <li class='reply_user'>
                                    <div class='reply_user_info'>
                                        ...
                                        <span class='reply_manage'>
                                            ...
                                            <div class='replyPwModal replyModiForm'>
                            ...
                            <?php
                            if(isset($_POST['rno'])){ // 만약 rno가 있다면 
                                $num = $_GET['num']; // 해당 글의 num을 가져옴 
                                $replyNum = $_POST['rno']; // 해당 댓글의 고유번호를 가져옴 
                                $sql = "SELECT * FROM boardReply WHERE replyNum=".$replyNum."";
                                $rs = mysqli_query($conn, $sql);
                                ...
                                if(isset($_POST['replyModifyPw'])){ // 만약 modifyPw 값이 있다면
                                    ...
                                    if(password_verify($modiPw, $encrypted_pw)){
                                        ...
                                        <div class='reply_form modiForm'>
                                            <p>< 댓글 수정란 ><p>
                                            <form action='./board_reply_modi.php?num=".$num."&rno=".$replyNum."' method='post'>
                                                <div class='reply_form_user'>
                                                    <input type='text' name='board_replyModi_name' value='".$info['id']."' required>
                                                    <input type='password' name='board_replyModi_pw' placeholder='비밀번호' required> 
                                                    <input type='hidden' name='rno' value='".$info['replyNum']."'>
                                                </div>
                                                <textarea name='board_replyModi_content'cols='30' rows='10' spellcheck='false'>".$info['content']."</textarea> <br>
                                                <input type='submit' value='작성'>
                                            </form>
                                        </div>
                                        ";
                                ...
                                if(isset($_POST['replyDeletePw'])){
                                    ...
                                    if(password_verify($deletePw, $encrypted_pw)){
                                        echo "<script>
                                        location.replace('./board_reply_delete.php?num=".$num."&rno=".$replyNum."');
                                        </script>";
                                    } else {
                                    echo "<script>
                                        alert('비밀번호가 일치하지 않습니다.');
