...
<?php
    ...
    $sql = "SELECT * FROM boardReply WHERE replyNum = ".$modiReply."";

    // form 데이터 받아오기 
    $name = $_POST['board_replyModi_name']; 
    $pw = $_POST['board_replyModi_pw']; 
    $content = $_POST['board_replyModi_content'];
    $date = date('Y-m-d');
    $replyNum = $modiReply;


    // sql문 작성 - 댓글 수정이므로 update문 사용  
    $sql = "UPDATE boardReply SET id = '".$name."',
    pw = '".$pw."',
    content = '".$content."',
